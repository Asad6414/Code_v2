...
data<-read_csv("https://raw.githubusercontent.com/Asad6414/Code_v2/main/data67-22.csv", show_col_types = FALSE)
...
data.bv <- cbind(gdp, cpi, prate, empration, wti_oil, divisia, gov_total, total_market)
data.bv

colnames(data.bv) <- c("gdp", "cpi", "prate", "empration", "wti_oil", "divisia", "gov_total", "total_market")

info.bv <- VARselect(data.bv, lag.max = 12, type = "both")
info.bv$selection

bv.est <- VAR(data.bv, p = 4, type = "both", season = NULL, exog = NULL)
summary(bv.est)

#To test for serial correlation we can apply a Portmanteau-test
bv.serial <- serial.test(bv.est, type = "BG")
bv.serial
...
#To load the data and store the time series objects
data<-read_csv("https://raw.githubusercontent.com/Asad6414/Code_v2/main/data_tr67-22.csv", show_col_types = FALSE)
# data<-read_csv("https://raw.githubusercontent.com/Asad6414/Code_v2/main/data67-22.csv", show_col_types = FALSE)
...
#To test for serial correlation we can apply a Portmanteau-test
bv_tr.serial <- serial.test(bv_tr.est, lags.pt = 12, type = "ES")
bv_tr.serial
...
info_tr.pz <- VARselect(data_tr.pz, lag.max = 12, type = "const", exogen = cbind(t, t2))
info_tr.pz$selection

pz_tr.est <- VAR(data_tr.pz, p = 5, type = "const", season = NULL, exog = cbind(t, t2))
summary(pz_tr.est)
